...
The newly introduced scoped enumerations fix the afforementioned issues with unscoped enums.
Firstly, they are strongly typed – they cannot be implicitly converted to `int`.
They can be explicitly converted to `int` using a `static_cast` if need be, but such cases are very specific.

Secondly, they are scoped – the type name acts as its own implicit namespace.
